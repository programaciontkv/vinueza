					...
					<table id="tbl_list" class="table table-bordered table-list table-hover table-striped" width="100%">
						...
						?>
							<tr>
								...
								<td style="mso-number-format:'@'"><?php echo $egreso->mov_documento?></td>
								<td style="mso-number-format:'@'"><?php echo $egreso->mov_guia_transporte?></td>
								<td><?php echo $egreso->cli_raz_social?></td>
								<td style="mso-number-format:'@'"><?php echo $egreso->mp_c?></td>
								<td><?php echo $egreso->mp_d?></td>
								<td><?php echo $egreso->mp_q?></td>
								<td><?php echo $egreso->trs_descripcion?></td>
								<td class="number"><?php echo str_replace(',','', number_format($egreso->mov_cantidad,$dcc))?></td>
								<td class="number"><?php echo str_replace(',','', number_format($egreso->mov_val_unit,$dec))?></td>
								<td class="number"><?php echo str_replace(',','', number_format($egreso->mov_val_tot,$dec))?></td>
								<?php
								if($grup!=$egreso->mov_documento){
									if($permisos->rop_reporte){
								?>
								<td>
										<a href="<?php echo base_url();?>egreso/show_frame/<?php echo $egreso->mov_documento?>/<?php echo $permisos->opc_id?>" class="btn btn-warning" title="RIDE"> <span class="fa fa-file-pdf-o" ></span></a>
								</td>		
								<?php 
									}
