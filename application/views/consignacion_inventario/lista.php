				...
				<div class="col-md-12">
					<table id="tbl_list" class="table table-bordered table-list table-striped" width="100%">
						...
						if(!empty($inventarios)){
							...
							$nom_a=$inventario->cli_raz_social;
							$nom_b=$inventario->tipo;

							//// total tipos 
							if(($inventario->mp_b!=$grup_b || $inventario->mov_tranportista!=$grup) && $n!=1){
								// if(round($inv_b,$dcc)!=0){	
									?>
								<tr  class="tr_t2"  id="tr_t2<?php echo $grup?>">
								<td colspan="3" ></td>
						...
						<?php
							...
							$inv_b=0;
								...
								//// total clientes 
							if($inventario->mov_tranportista != $grup && $n!=1 ){
						?>
								<tr class="tr_t1" id="tr_t1<?php echo $grup?>">
									<td colspan="3" id="<?php echo 'a_'.$grup?>"  class="acordeon" ></td>
						...
						<?php
							 $inv_a=0;
								 ...
								 if($inventario->mov_tranportista != $grup){
									?>
								<tr class="tr_familia">
									<td id="<?php echo 'a_'.$inventario->mov_tranportista?>"  lang="<?php echo $n?>"class="acordeon" onclick="mostrar(1,<?php echo $inventario->mov_tranportista?>,this)">-</td>
							...
							if($inventario->mp_b!=$grup_b || $inventario->mov_tranportista!=$grup ){
								// if(round($inv_b,$dcc)!=0){	
						?>
								<tr class="tr_tipo <?php echo 'tra_'.$inventario->mov_tranportista?>">
									<td id="<?php echo 'b_'.$inventario->mp_b?>" lang="<?php echo $n?>" class="acordeon <?php echo 'tda_'.$inventario->mp_b?> tb" onclick="mostrar(0,<?php echo $inventario->mp_b?>,this,<?php echo $inventario->mov_tranportista?>)">-</td>
						...
						<?php
							...
							if(round($inv,$dcc)!=0){	
						?>
							<tr class="tr <?php echo 'b_'.$inventario->mp_b.'_a_'.$inventario->mov_tranportista ?> <?php echo 'a_'.$inventario->mov_tranportista ?>  ">
								<td></td>
						...
						<?php
							...
							$grup=$inventario->mov_tranportista;
							$grup_b=$inventario->mp_b;
							$inv_a+=round($inv,$dcc);
							$nom_a=$inventario->cli_raz_social;
							$nom_a1=$inventario->cli_raz_social;
						...
						?>
								<tr class="tr_t1" id="tr_t1<?php echo $inventario->mov_tranportista?>">
									<td colspan="3" id="<?php echo 'a_'.$inventario->mov_tranportista?>" class="acordeon" ></td>
									<td  style="mso-number-format:'@'">TOTAL <?php echo $nom_a1?>  </td>
									<td class="number"><?php echo str_replace(',','', number_format($inv_a,$dcc))?></td>
								</tr>
						<?php
...
<script type="text/javascript">
	...
	function mostrar(opc, id,obj,cli){
		if(opc==0){
			if($(obj).html()=='+'){
				$(obj).html('-');
				if($('#detalle').prop('checked')==true){
