<section class="content-header">
	<form id="exp_excel" style="float:right;padding:0px;margin: 0px;" method="post" action="<?php echo base_url();?>ingreso/excel/<?php echo $permisos->opc_id?>/<?php echo $fec1?>/<?php echo $fec2?>" onsubmit="return exportar_excel()"  >
		...
		<div class="box box-body">
			...
			<div class="row">
				<div class="col-md-1">
					...
					?>
						<a href="<?php echo base_url();?>ingreso/nuevo/<?php echo $opc_id?>" class="btn btn-primary btn-flat"><span class="fa fa-plus"></span> Nuevo</a>
					<?php 
					}
					?>
				</div>	
					...
					<table id="tbl_list" class="table table-bordered table-list table-hover table-striped" width="100%">
						...
						?>
							<tr>
								...
								<td style="mso-number-format:'@'"><?php echo $ingreso->mov_num_fac_entrega?></td>
								<td><?php echo $ingreso->cli_raz_social?></td>
								<td style="mso-number-format:'@'"><?php echo $ingreso->mp_c?></td>
								<td><?php echo $ingreso->mp_d?></td>
								<td><?php echo $ingreso->mp_q?></td>
								<td><?php echo $ingreso->trs_descripcion?></td>
								<td class="number"><?php echo str_replace(',','', number_format($ingreso->mov_cantidad,$dcc))?></td>
								<td class="number"><?php echo str_replace(',','', number_format($ingreso->mov_val_unit,$dec))?></td>
								<td class="number"><?php echo str_replace(',','', number_format($ingreso->mov_val_tot,$dec))?></td>
							</tr>
						<?php
							}
						}
