...
class pdf_estado_cta_cobros_model extends CI_Model {
	...
	public function lista_facturas_cliente($id, $emp,$fecha){
	...
	public function lista_facturas_cliente_2($id, $emp,$fecha){
		$query ="select emp_id,fac_id,cli_id, fac_fecha_emision,fac_numero,fac_total_valor,fac_fecha_emision, pago as credito,(select pag_fecha_v from pagos where fac_id=$id order by pag_fecha_v desc limit 1) as pag_fecha_v from pagos where pag_fecha_v<='$fecha' 
		and fac_fecha_emision<='$fecha' and cli_id=$id and emp_id=$emp group by emp_id,fac_id,cli_id, fac_fecha_emision,fac_numero,fac_total_valor, fac_fecha_emision, pago ORDER BY fac_numero ASC ";
        $resultado=$this->db->query($query);
		return $resultado->result();
