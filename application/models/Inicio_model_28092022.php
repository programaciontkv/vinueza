...
class Inicio_model extends CI_Model {
	...
	public function datos_pro($fecha){
		...
		$this->db->where("f.pro_id = p.id and f.fac_id=ef.fac_id and ef.fac_fecha_emision >= '$fecha' and p.ids='26' group by p.mp_d,p.id order by nu desc limit 10 ",null);
    ...
    // GROUP BY fech
		$this->db->select("to_char(ef.fac_fecha_emision ,'mm') AS fech, SUM(ef.fac_total_valor) as total");
