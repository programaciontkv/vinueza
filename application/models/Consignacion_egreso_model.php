...
class Consignacion_egreso_model extends CI_Model {
	...
	public function lista_egresos_buscador($text,$ids,$f1,$f2,$txt){
		$this->db->select("m.*,e.est_descripcion, c.cli_raz_social, t.trs_descripcion, p.id, p.mp_a,p.mp_b,p.mp_c,p.mp_d,p.mp_q,em.emi_nombre" );
		$this->db->from('erp_i_mov_inv_pt m');
		$this->db->join('erp_transacciones t','t.trs_id=m.trs_id');
		$this->db->join('erp_mp p','p.id=m.pro_id');
		$this->db->join('erp_estados e','e.est_id=m.mov_estado');
		$this->db->join('erp_emisor em','em.emi_cod_cli=m.cli_id');
		$this->db->join('erp_i_cliente c','c.cli_id=cast(m.mov_tranportista as integer)');
		$this->db->where("(mov_documento like '%$text%' or cli_raz_social like '%$text%' or mp_c like '%$text%' or mp_d like '%$text%') and ids='$ids' and mov_fecha_trans between '$f1' and '$f2' $txt and mp_i='1' and mov_estado='1' and trs_operacion=1", null);
		$this->db->order_by('m.mov_fecha_trans','desc');
		$this->db->order_by('m.mov_documento','desc');
		$resultado=$this->db->get();
		return $resultado->result();
	...
	public function total_ingreso_egreso_fact_cli($id, $txt, $cli) {
       
        $query ="select(SELECT SUM(m.mov_cantidad)as suma FROM erp_i_mov_inv_pt m, erp_transacciones t WHERE m.trs_id=t.trs_id and m.pro_id=$id and t.trs_operacion= 0 $txt and mov_estado=1 and mov_tranportista='$cli') as ingreso,
                                   (SELECT SUM(m.mov_cantidad)as suma FROM erp_i_mov_inv_pt m, erp_transacciones t WHERE m.trs_id=t.trs_id and m.pro_id=$id  and t.trs_operacion= 1 $txt and mov_estado=1 and mov_tranportista='$cli') as egreso";
	...
	public function lista_un_egreso_secuencial($id){
		$this->db->select('m,*, p.*,em.*,e.emi_nombre as origen, d.emi_nombre as destino');
		$this->db->from('erp_i_mov_inv_pt m');
		$this->db->join('erp_mp p','m.pro_id=p.id');
		$this->db->join('erp_emisor d','m.cli_id=d.emi_cod_cli');
		$this->db->join('erp_i_cliente c','cast(m.mov_tranportista as integer)=c.cli_id');
		...
		$this->db->where('m.trs_id','20');
		$resultado=$this->db->get();
		return $resultado->row();
	}

	public function lista_detalle_egreso_secuencial($id){
		$this->db->from('erp_i_mov_inv_pt m');
		$this->db->join('erp_mp p','m.pro_id=p.id');
		$this->db->join('erp_emisor e','m.bod_id=e.emi_id');
		$this->db->join('erp_empresas em','e.emp_id=em.emp_id');
