...
class Reg_retencion_model extends CI_Model {
	...
	public function lista_saldo_factura($id){
		$query ="select (select fac_total_valor from erp_factura where fac_id=$id) as total,
                        (select sum(cta_monto) from erp_ctasxcobrar where com_id=$id and cta_estado=1) as credito
                                    ";
