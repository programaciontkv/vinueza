...
class Backend_lib {
	...
	public function control($opc_id=''){
		...
		if($url=='factura' || $url=='nota_credito' || $url=='guia_remision' || $url=='ingreso' || $url=='egreso'|| $url=='pedido_factura'|| $url=='pedido'){
			$accion=$this->CI->uri->segment(2);
			if($accion=='nuevo' || $accion=='guardar' || $accion=='show_frame' || $accion=='show_ftalon'){
