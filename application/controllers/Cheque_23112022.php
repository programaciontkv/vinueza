...
class Cheque extends CI_Controller {
	...
	public function cobrar($id,$opc_id,$cli){
		...
		$cheque=$this->cheque_model->lista_un_cheque2($id);
		$rst_pln=$this->plan_cuentas_model->lista_un_plan_cuentas_codigo(trim($cheque->chq_cuenta));
		if($this->permisos->rop_actualizar){
			$data=array(
						'dec'=>$dec,
						'titulo'=>ucfirst(strtolower($rst_cja->emp_nombre)),
	...
	public function guardar_cobros($opc_id){
		...
		$asiento = array(
			     'con_asiento'       =>  $asi,
			     'con_concepto'      =>  'CONTROL DE COBROS',
