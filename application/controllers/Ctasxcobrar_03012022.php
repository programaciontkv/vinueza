...
class Ctasxcobrar extends CI_Controller {
    ...
    public function show_frame2($id,$opc_id){
    	...
    	if($permisos->rop_reporte){
    		$data=array(
					'titulo'=>'Cuentas Por Cobrar '.ucfirst(strtolower($rst_cja->emi_nombre)),
					'regresar'=>strtolower($rst_opc->opc_direccion).$rst_opc->opc_id,
					'direccion'=>"ctasxcobrar/show_pdf2/$id/$opc_id",
				);
			$this->load->view('layout/header',$this->menus());
			$this->load->view('layout/menu',$this->menus());
			$this->load->view('pdf/frame',$data);
    ...
    public function show_frame3($id,$opc_id,$fecha){
		$permisos=$this->backend_model->get_permisos($opc_id,$this->session->userdata('s_rol'));
		$rst_opc=$this->opcion_model->lista_una_opcion($opc_id);
		$rst_cja=$this->caja_model->lista_una_caja($rst_opc->opc_caja);
    	...
    	if($permisos->rop_reporte){
    		$data=array(
					'titulo'=>'Cuentas Por Cobrar '.ucfirst(strtolower($rst_cja->emi_nombre)),
					'regresar'=>strtolower($rst_opc->opc_direccion).$rst_opc->opc_id,
					'direccion'=>"ctasxcobrar/show_pdf3/$id/$opc_id/$fecha",
	...
	public function show_pdf2($fac_id,$opc_id,$fecha){
			$rst_fac=$this->factura_model->lista_una_factura($fac_id);
			$conf=$this->configuracion_model->lista_una_configuracion('2');
			$dec=$conf->con_valor;
			$data=array(
						...
						// $cns =>$this->cliente_model->lista_estado_cuenta_cliente($rst_fac->cli_id, $fec1, $fec2);
			...
			$this->html2pdf->filename('ctasxcobrar_factura.pdf');
			$this->html2pdf->paper('a4', 'landscape');
    		$this->html2pdf->html(utf8_decode($this->load->view('pdf/pdf_estado_cuenta', $data, true)));
    		//$this->html2pdf->output(array("Attachment" => 0));

	}

	public function show_pdf3($fac_id,$opc_id,$fecha){
			$rst_fac=$this->factura_model->lista_una_factura($fac_id);
			$conf=$this->configuracion_model->lista_una_configuracion('2');
			$rst_opc=$this->opcion_model->lista_una_opcion($opc_id);
			$rst_cja=$this->caja_model->lista_una_caja($rst_opc->opc_caja);
			...
			$data=array(
						...
						'cliente' => $this->cliente_model->lista_un_cliente($rst_fac->cli_id),
						'cns' => $this->cliente_model->lista_facturas_cliente($rst_fac->cli_id),
						'fec_act'=>$fec_act,
						'logo' => $emp->emp_logo,
						'cns_det'=>$this->ctasxcobrar_model->lista_ctasxcobrar($rst_fac->fac_id),
						'cns_pag'=>$this->ctasxcobrar_model->lista_pagos_factura($rst_fac->fac_id),
			...
			$this->html2pdf->filename('ctasxcobrar_factura.pdf');
			$this->html2pdf->paper('a4', 'landscape');
			$this->load->view('pdf/pdf_std_cta_cliente_pagos', $data);
    		//$this->html2pdf->html(utf8_decode($this->load->view('pdf/pdf_std_cta_cliente_pagos', $data, true)));
    		//$this->html2pdf->output(array("Attachment" => 0));
		...
		$rst_cja=$this->caja_model->lista_una_caja($rst_opc->opc_caja);
            ...
            switch ($reporte) {
                case '0':
                    ...
                    break;
                case '1':
                    $url="pdf_estado_cliente/index/$id/$opc_id/$fec";
                    break;
                case '2':
                ...
                case '4':
                    ...
                    break;  
                case '5':
                    $url="pdf_estado_cta_cliente/index/$id/$opc_id/$fec";
                    break;
                case '6':
                    $url="pdf_estado_cta_cobros/index/$id/$opc_id";
                    break;              
            }
