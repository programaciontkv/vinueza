...
class Cheque extends CI_Controller {
	...
	public function index($opc_id){
		...
		if($_POST){
			...
			$cns_cheques=$this->cheque_model->lista_cheque_buscador($text,$f1,$f2,$rst_cja->emp_id);
		}else{
			$text= '';
			$f1= date('Y-m-d');
			$f2= date('Y-m-d');
	...
	public function show_frame($id,$opc_id){
    	...
    	if($permisos->rop_reporte){
    		$data=array(
					'titulo'=>'Cuentas Por Cobrar '.ucfirst(strtolower($rst_cja->emp_nombre)),
    ...
    public function show_pdf($fac_id,$opc_id){
			...
			$data=array(
						...
						'credito'=>0,
						'saldo_vencido'=>$this->cheque_model->lista_saldo_factura($rst_fac->fac_id),
						'cns_pag'=>$this->cheque_model->lista_pagos_factura($rst_fac->fac_id),
						'cns_det'=>$this->cheque_model->lista_cheque($rst_fac->fac_id),
						);

			$this->html2pdf->filename('cheque_factura.pdf');
			$this->html2pdf->paper('a4', 'landscape');
    		$this->html2pdf->html(utf8_decode($this->load->view('pdf/pdf_cheque_factura', $data, true)));
