...
class Cheque extends CI_Controller {
	...
	public function cobrar($id,$opc_id,$cli){
		...
		{
			$cuenta_anterior=intval($cuenta_anterior->pln_id);
		}
		if($this->permisos->rop_actualizar){
			$data=array(
						'dec'=>$dec,
	...
	public function guardar_cobros($opc_id){
		...
		$asiento = array(
			     'con_asiento'       =>  $asi,
			     'con_concepto'      =>  'CONTROL DE COBROS- BANCO',
			     'con_documento'     =>  '', //doc
	  	...
	  	/////asiento de abono
			...
			if ($saldo>0) {
				$asiento = array(
				     'con_asiento'       =>  $asi,
				     'con_concepto'      =>  'CONTROL DE COBROS-ABONO',
		...
		while($n<$count){
			$n++;
			$fac_id        =  $this->input->post("fac_id$n");
			$cli_id        =  $this->input->post("cli_id$n");
			$fac_num       =  $this->input->post("fac_num$n");
			$cobro         =  $this->input->post("cobro$n");
			if($cobro>0){
			$asiento = array(
			     'con_asiento'       =>  $asi,
			     'con_concepto'      =>  'CONTROL DE COBROS - FACTURAS',
			     'con_documento'     =>  $fac_num, //doc
			     'con_fecha_emision' =>  $fecha_emision, //doc
			     'con_concepto_debe' =>  '', //doc
			     'con_concepto_haber'=>  '1.01.02.01.001', //doc
			     'con_valor_debe'    =>  0, //doc
    ...
    public function show_pdf($id,$opc_id){
			$cns_cxc=$this->ctasxcobrar_model->lista_ctasxcobrar_cheque($id);
			$cxc=array();
