...
class otro extends CI_Controller {
	...
	public function index(){
		$data=array(
					...
					'productos'=>$this->otro_model->lista_productos()
				);
	...
	public function guardar(){
		...
		if($this->form_validation->run()){
			...
			if($this->otro_model->insert($data)){
				...
				redirect(base_url().'otro');
			
			}else{
				$this->session->set_flashdata('error','No se pudo guardar');
				redirect(base_url().'otro/nuevo');
			}
		}else{
			$this->nuevo();
		}	
	...
	public function editar($id){
		if($this->permisos->rop_actualizar){
			...
			$data=array(
						...
						'action'=>base_url().'otro/actualizar'
						);
	...
	public function actualizar(){
		...
		if($this->form_validation->run()){
			...
			if($this->otro_model->update($id,$data)){
				...
				redirect(base_url().'otro/editar'.$id);
			}
		}else{
			$this->editar($id);
		}	
