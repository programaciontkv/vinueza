...
class Ingreso extends CI_Controller {
	...
	public function nuevo($opc_id){
		...
		if($permisos->rop_insertar){
			...
			$data=array(
						...
						'transacciones'=>$this->ingreso_model->lista_transacciones(),
						...
						'ingreso'=> (object) array(
					                        ...
					                        'mov_num_fac_entrega'=>'',
					                        'trs_id'=>'28',
					                        'trs_descripcion'=>'INGRESO POR COMPRAS',
					                        'cli_nombre'=>'',
					                        'cli_id'=>'0',
	...
	public function guardar($opc_id){
		...
		$mov_num_fac_entrega = $this->input->post('mov_num_fac_entrega');
		    	...
		    	while($n<$count_det){
		    		...
		    		if($this->input->post("pro_id$n")!=''){
						...
						$data=array(
					                ...
					                'mov_num_fac_entrega'=>$mov_num_fac_entrega,
						...
						if($this->ingreso_model->insert($data)){
							...
							$data=array(	
								...
								'reg_femision'=>$mov_fecha_trans,
								'reg_fautorizacion'=>$mov_fecha_trans,
								'reg_fcaducidad'=>$mov_fecha_trans,
								...
								'reg_forma_pago'=>'0',
								'reg_tpcliente'=>'0', 
								'reg_num_documento'=>$mov_num_fac_entrega,
								'reg_num_registro'=>$secuencial,
								'reg_estado'=>'7'
		    				);
						    ...
						    if(!empty($fac_id)){
						    	...
						    	while($n<$count_det){
						    		...
						    		if($this->input->post("pro_id$n")!=''){
						    			$pro_id = $this->input->post("pro_id$n");
						    			$dfc_codigo = $this->input->post("pro_codigo$n");
						    			$dfc_cod_aux = $this->input->post("pro_codigo$n");
						    			$dfc_cantidad = $this->input->post("mov_cantidad$n");
						    			$dfc_precio_unit = $this->input->post("mov_cost_unit$n");
						    			$dfc_precio_total = $this->input->post("mov_cost_tot$n");
						    			$dt_det=array(
						    							'reg_id'=>$fac_id,
					                                    'pro_id'=>$pro_id,
	...
	public function excel($opc_id,$fec1,$fec2){
    	...
    	$titulo='Ingresos '.ucfirst(strtolower($rst_cja->emi_nombre));
