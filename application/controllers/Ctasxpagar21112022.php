...
class Ctasxpagar extends CI_Controller {
    ...
    public function show_pdf($fac_id,$opc_id){
			...
			$data=array(
						...
						'cliente'=>$this->cliente_model->lista_un_cliente($rst_fac->cli_id),
						'credito'=>0,
						'saldo_vencido'=>$this->ctasxpagar_model->lista_saldo_factura($rst_fac->fac_id),
						'cns_pag'=>$this->ctasxpagar_model->lista_pagos_factura($rst_fac->fac_id),
						'cns_det'=>$this->ctasxpagar_model->lista_ctasxpagar($rst_fac->fac_id),
						);
