...
class Kardex extends CI_Controller {
				...
				//totales
					if($grup!=$kardex->pro_id && $n!=1){
						...
						$detalle.="<tr class='success'>
								...
								<td class='h-mobile' class='number total' style='font-weight: bolder;text-align:center'>". str_replace(',','', number_format($t_ctu_ing,$dec))."</td>
								...
								<td class='h-mobile' class='number total' style='font-weight: bolder;text-align:center'>". str_replace(',','', number_format($t_ctu_egr,$dec))."</td>
								...
								<td class='h-mobile' class='number total' style='font-weight: bolder;text-align:center'>". str_replace(',','', number_format($t_sal_uni,$dec))."</td>
    ...
    public function show_frame($opc_id){
		...
		///buscador 
			...
			$ids= '26';
			$f1= $this->input->post('fecha1');
			$f2= $this->input->post('fecha2');

    	if($permisos->rop_reporte){
    		$data=array(
					'titulo'=>'Kardex ',
					'regresar'=>strtolower($rst_opc->opc_direccion).$rst_opc->opc_id,
					'direccion'=>"kardex/show_pdf/$opc_id/$ids/$f1/$f2/$text",
				);
			$this->load->view('layout/header',$this->menus());
			$this->load->view('layout/menu',$this->menus());
