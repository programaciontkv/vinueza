...
class Egreso extends CI_Controller {
	...
	public function nuevo($trs_id,$opc_id){
		...
		if($permisos->rop_insertar){
			...
			if($we>=760){
				...
				$this->load->view('egreso/form_movil',$data);
			}
	...
	public function load_producto($id,$emi){
		...
		if(!empty($rst)){
			...
			$rst1 =$this->egreso_model->total_ingreso_egreso_fact($rst->id,$fra); 
			if(!empty($rst1)){
    ...
    public function show_pdf($opc_id,$id){
			...
			$egreso=$this->egreso_model->lista_un_egreso_secuencial($id);
			$direccion= $egreso->emp_ciudad."-".$egreso->emp_pais;
			$this->html4pdf->filename('egreso.pdf');
			$this->html4pdf->paper('a4', 'portrait');
			$this->html4pdf->empresa($egreso->emp_nombre,$egreso->emp_identificacion,$direccion,$egreso->emp_telefono,$egreso->emp_logo);
    		$this->html4pdf->html(utf8_decode($this->load->view('pdf/pdf_egreso', $data, true)));
