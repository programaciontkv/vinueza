...
class Consignacion_inventario extends CI_Controller {
	...
	function __construct(){
		...
		$this->load->model('consignacion_inventario_model');
	...
	public function index($opc_id){
		...
		if($_POST){
			...
			$cns_mov=$this->consignacion_inventario_model->lista_inventarios_buscador($text,$ids,$f2,$txt);						
			if($fam=='on'){
			$fam='checked';
			}else{
				$fam='';
		...
		$this->load->view('consignacion_inventario/lista',$data);
	...
	public function excel($opc_id,$fec1,$fec2){
    	...
    	$titulo='Inventario Consignacion';
    ...
    public function reporte($opc_id,$fec1,$fec2){
    	...
    	$titulo='Inventario Consignacion'.ucfirst(strtolower($rst_cja->emi_nombre));
    	$file="inventario".date('Ymd');
    	$data=$_POST['datatodisplay'];
    	$this->export_excel->to_excel3($data,$file,$titulo,$fec1,$fec2);
    }
    ...
    public function show_frame($opc_id){
		...
		$rst_cja=$this->caja_model->lista_una_caja($rst_opc->opc_caja);
			...
			echo "consignacion_inventario/show_pdf/$opc_id/$ids/$fec2/$fam/$tip/$det/$text";
    	if($permisos->rop_reporte){
    		$data=array(
					'titulo'=>'Inventario Consignacion',
					'regresar'=>base_url().strtolower($rst_opc->opc_direccion).$rst_opc->opc_id,
					'direccion'=>"consignacion_inventario/show_pdf/$opc_id/$ids/$fec2/$fam2/$tip2/$det2/$text",
					'fec1'=>$fec1,
					'fec2'=>$fec2,
					'txt'=>$text,
					'estado'=>'',
