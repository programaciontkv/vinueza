...
class Pdf_estado_cta_cobros extends CI_Controller {
	...
	function __construct(){
	 ...
	 public function index($id,$opc_id){
        ...
        $cns = $this->pdf_estado_cta_cobros_model->lista_facturas_cliente($cliente->cli_id,$rst_cja->emp_id);
        $fec1='2000-01-01';
        $fec2=date('Y-m-d');
       
        
        $pdf->Image('./imagenes/'.$emisor->emp_logo, 6, 4, 30);
        $pdf->Ln();  
        $pdf->SetFont('helvetica', 'B', 12);
        $pdf->SetTextColor(0, 160, 186);
        $pdf->Cell(200, 5, "CUENTAS POR COBRAR ", 0, 0, 'C');
        $pdf->Ln();     
        $pdf->Cell(200, 5, "ESTADO DE CUENTA CLIENTE AL: " . $fec2, 0, 0, 'C');
        $pdf->Ln();
