...
class Servicio extends CI_Controller {
	...
	public function index($opc_id){
		...
		if($_POST){
			...
			$cns_productos=$this->servicio_model->lista_productos_buscador($text,$txt_est);
		}else{
			$text= '';
			$txt_est="1";
			$est="1";
	...
	public function nuevo($opc_id){
		...
		if($this->permisos->rop_insertar){
			...
			$data=array(
						...
						'action'=>base_url().'servicio/guardar/'.$opc_id
						);
	...
	public function guardar($opc_id){
		...
		if($this->form_validation->run()){
			...
			if($this->servicio_model->insert($data)){
				...
				redirect(base_url().'servicio/'.$opc_id);
	...
	public function editar($id,$opc_id){
		...
		if($this->permisos->rop_actualizar){
			...
			$data=array(
						...
						'action'=>base_url().'servicio/actualizar/'.$opc_id,
						'opc_id'=>$rst_opc->opc_id
						);
    ...
    public function traer_codigo($id1,$id2){
    	...
    	if($conf->con_valor==0){
    		//$pro    = $this->servicio_model->lista_by_tipo($id1,$id2);
    		$pro    = $this->servicio_model->lista_by_tipo_count($id1,$id2);
	        $rst_s1 = $this->tipo_model->lista_un_tipo($id1);
            $rst_s2 = $this->tipo_model->lista_un_tipo($id2);
	        
           if(!empty($pro->n)){
           	 $sec = $pro->n + 1;
           	 // echo substr($pro->mp_c, -3);
           }else{
           	$sec=1;
           }
            if ($sec > 0 && $sec < 10) {
